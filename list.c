...
bool list_less(const struct list_elem *a, const struct list_elem *b, void *aux)
{
  ...
  if(strcmp((char *)aux, "ascending") == 0)
    return a_data < b_data;

  else if (strcmp((char *)aux, "descending") == 0)
    return a_data > b_data;
...
list_shuffle(struct list *list)
{
  srand(time(NULL));
    ...
    if(size < 2) return;  // 요소가 1개 이하면 셔플 필요 없음

    for(size_t i = 0; i < size; i++) {
        size_t rand_num1 = rand() % size;
        size_t rand_num2 = rand() % size;

        struct list_elem *first = list_begin(list);
        struct list_elem *second = list_begin(list);

        for(size_t j = 0; j < rand_num1; j++) {
            first = list_next(first);
        }

        for(size_t j = 0; j < rand_num2; j++) {
            second = list_next(second);  // 여기서 first 대신 second 사용
        }
