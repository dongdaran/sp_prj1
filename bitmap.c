...
/* Returns an elem_type where only the bit corresponding to
   BIT_IDX is turned on. -> 필요한 비트 인덱스 나타내기*/
...
{
  size_t idx = elem_idx (bit_idx); //배열의 위치
...
/* 해당 elem_type index & elem_type bit index에 value가 있는지 확인*/
bool
...
//특정 bit pattern 찾기 및 bit pattern 0->1 & 1->0 반전
...
struct bitmap *bitmap_expand(struct bitmap *bitmap, int size)
{
  if(bitmap != NULL)
    { 
      size_t prev = bitmap_size(bitmap);
      bitmap->bit_cnt +=  size;
      bitmap->bits = realloc(bitmap->bits, byte_cnt(bitmap->bit_cnt));
      bitmap_set_multiple(bitmap, prev, size, false);
...
/*20231523*/
void 
bitmap_print_binary(struct bitmap *b) 
{
    ASSERT(b != NULL);
